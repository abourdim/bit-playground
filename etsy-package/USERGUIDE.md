...
├── BitPlayground-v*.zip         📦 Final ZIP (this is what the buyer downloads)
│
...
- **What:** Node script that renders the 6 printable HTML templates to
  ...
  everything into `BitPlayground-v<version>.zip`.
...
- **Outputs:**
  ...
  - `etsy-package/BitPlayground-v<version>/` (staging dir, rebuilt)
  - `etsy-package/BitPlayground-v<version>.zip` (the buyer ZIP)
...
11. Attach the digital file: `BitPlayground-v<version>.zip`.
...
17. Keep old ZIPs tracked in Git (the `.gitignore` exemption keeps
    `etsy-package/BitPlayground-v*.zip` in history) so you can always
